...
1. `Screen.getByText("XYZ")` : Finds an element in the DOM that contains the specified text.
...
10. `queryFallbacks`: By default, only the first role of an element is considered
    ...
    getByRole('switch') will work but what if we want to take the fallback role which is checkbox then
...
5. `Default Normalization`:
   ...
   if you don't want to use then you can edit few setting in Default Normalization or provide your custom Normalization function.
...
1. `.toBeDefined()`: Use .toBeDefined to check that a variable is not undefined. 2.` .toBeFalsy()`: this matches anything that an if statement treats as false.
...
1. `render`
   ...
   byDefault container is = <div<span></span>><div<span></span>>

   so if you render(<div<span></span>/>) then your created element is automatically appended to
   the container document.body.appendChild(<div<span></span>/>) the output will be:
   ...
   1. `container` : By default, React Testing Library will create a div and append
...
3.  renderHook: this method is designed for testing your custom hooks in testing
    ...
    2. return methods that you can use on renderHook
...
1.  `screen.logTestingPlaygroundURL()`
    ...
    1.  first panel is a code editer which has your render HTML on it
        automatically that was copied from your code.

    2.  second panel is the web representation of your HTML Code to select which
    ...
    3.  third panel show a 100% gurenty way to select the element which you should
        use when nothing works out.

    4.  fourth panel show you role suggestion you should use/ (this is the most
...
2.  `Fallback query`
    ...
    2.2. container.querySelector(): this is the last resort. if nothing works.
    ...
    container. container is outer level HTML element. which is by default a <div<span></span>>
...
2. your custom matcher fn should do the testing but return 2 cases
   ...
   2.1. pass: true when expected result is returned
   2.2. pass: false when expected result not match with that also provide a
   message for screen.debug to show message why expected value fail.

3. in your custom component your first argunment is the element that you get in
   your expect(element) test.
4. use custom matcher when you are finding a lot of repeated test cases you need
...
2. you can use it as constructor as new RegEx(pattern,flag)
   ...
   1. pattern can be regular expression as a string (also can be dynamic)
   2. flag (optional) there are some filter condion that help to manupulated
...
[RegEx MDN](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/RegExp/RegExp)

---

## Extra: javascript debugger keyword
